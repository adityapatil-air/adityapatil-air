### Hi, I'm Aditya 👋

Computer Engineering undergraduate at MIT Academy of Engineering, Pune.
I build end-to-end systems that move and transform data — pipelines, cloud
infrastructure, and the integrations that hold them together.

**Currently focused on**
- Data pipeline architecture (ingestion → processing → aggregation → serving)
- AWS cloud infrastructure (S3, Lambda, CloudTrail, EC2, IAM)
- System design, edge-case handling, and reliable distributed workflows

**Recent**

Building [HandsOnCloud](https://github.com/adityapatil-air/HandsOnCloud) —
 I'm building HandsOnCloud — it parses AWS CloudTrail logs through a scoring engine and turns your cloud activity into a GitHub-style heatmap. Each action is scored by impact.

The problem I'm solving: freshers claim AWS hands-on experience on resumes but recruiters have no way to verify who actually did real work vs who just listed it. HandsOnCloud gives verifiable proof directly from CloudTrail data.

AWS-only for now, planning to go cloud-agnostic — GCP and Azure logs next.


**Tech I work with**
`Python` `JavaScript` `C++` `AWS` `Docker` `FastAPI` `Flask` `React`
`PostgreSQL` `MongoDB` `Hugging Face` `Linux`

**Reach me**
📧 adityapatil.air@gmail.com
